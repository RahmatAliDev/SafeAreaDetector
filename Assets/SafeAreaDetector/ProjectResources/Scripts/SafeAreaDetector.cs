...
namespace SAD
{
    ...
    public class SafeAreaDetector : MonoBehaviour
    {
        ...
        [Space, Header("Should Use In Awake")]
        public bool _shouldUseInAwake = false;
        public bool _shouldAdjustOnSpaceBarPressed = false;

        private void Awake()
        {
            // Initialize the safe area adjustment when the script is loaded
            if (_shouldUseInAwake)
            {
        ...
        private void OnGUI()
        {
            if (Event.current != null && Event.current.type == EventType.KeyDown && Event.current.keyCode == KeyCode.Space
                && _shouldAdjustOnSpaceBarPressed)
            {
                Debug.Log("Space key pressed in Edit Mode!");
                SetDataForDeviceSafeArea();
            }
        }
